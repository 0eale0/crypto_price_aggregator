...
router_cryptocurrency = SQLAlchemyCRUDRouter(
    schema=crud.Cryptocurrency, db_model=users.Cryptocurrency, db=get_session, dependencies=[Depends(is_admin)]
)

router_user = SQLAlchemyCRUDRouter(
    schema=crud.User, db_model=users.User, db=get_session, dependencies=[Depends(is_admin)]
)
...
router_exchange = SQLAlchemyCRUDRouter(
    schema=crud.Exchange, db_model=users.Exchange, db=get_session, dependencies=[Depends(is_admin)]
)

router_coin_price = SQLAlchemyCRUDRouter(
    schema=crud.CoinPrice, db_model=users.CoinPrice, db=get_session, dependencies=[Depends(is_admin)]
)
...
router_post = SQLAlchemyCRUDRouter(
    schema=crud.Post, db_model=users.Post, db=get_session, dependencies=[Depends(is_admin)]
)

router_post_picture = SQLAlchemyCRUDRouter(
    schema=crud.PostPicture, db_model=users.PostPicture, db=get_session, dependencies=[Depends(is_admin)]
)

router_like = SQLAlchemyCRUDRouter(
    schema=crud.Like, db_model=users.Like, db=get_session, dependencies=[Depends(is_admin)]
)

router_posts_comment = SQLAlchemyCRUDRouter(
    schema=crud.PostsComment, db_model=users.PostsComment, db=get_session, dependencies=[Depends(is_admin)]
)
